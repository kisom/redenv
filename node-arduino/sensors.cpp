...
Log(String line)
{
  ...
  dataFile.println(String(when) + ": " + line);
}
...
takeReading(struct Reading *r)
{
  ...
  now(r);
  lastReadingTime = rtc.now().unixtime();
...
SensorInit()
{
  ...
  if (SD.begin(SD_CS)) {
#ifdef DISABLE_SD_LOGGING
    Serial.println("SD card detected; SD logging is disabled.");
#else
